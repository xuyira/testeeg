...
class ExP():
    def __init__(self, nsub, total_sub=9):
        ...
        self.nSub = nsub
        self.total_sub = total_sub  # 总受试者数量
        
        # LOSO训练优化参数
        self.patience = 200  # 早停耐心值：200个epoch内测试准确率无提升则停止
        self.min_delta = 0.0001  # 最小改进阈值
    ...
    def get_source_data(self):
        # Cross-subject validation: Leave-One-Subject-Out (LOSO)
        # Test on subject nSub, train on all other subjects
        
        train_data_list = []
        train_label_list = []
        
        # Load training data from all subjects except the test subject
        for subject_id in range(1, self.total_sub + 1):
            if subject_id == self.nSub:
                continue  # Skip the test subject
            
            # Load both T and E data for training subjects
        ...
        # Load test data from the test subject (both T and E)
        test_data_list = []
        test_label_list = []
        
        for data_type in ['T', 'E']:
            file_path = self.root + 'A0%d%s.mat' % (self.nSub, data_type)
    ...
    def train(self):
        ...
        for e in range(self.n_epochs):
            ...
            if (e + 1) % 1 == 0:
                ...
                if acc > bestAcc + self.min_delta:
                    ...
                    Y_pred = y_pred
                    # 保存最佳模型
                    torch.save(self.model.module.state_dict(), 'best_model_subject%d.pth' % self.nSub)
                else:
                    epochs_no_improve += 1
        ...
        # 训练结束，加载最佳模型
        torch.save(self.model.module.state_dict(), 'final_model_subject%d.pth' % self.nSub)
...
def main():
    ...
    for i in range(9):
        ...
        print('Subject %d' % (i+1))
        exp = ExP(i + 1, total_sub=9)  # 指定总受试者数量
